...
plot(evals$age, evals$bty_avg)#age does not seem to provide any association
```
...
ggplot(data = evals, aes(x = bty_avg, y = score)) + geom_jitter() + geom_smooth(method = "lm", se = TRUE)
```
...
<ol>
<li> 0.19 points lower than minority professors, all else held constant. </li>
<li> **0.19 points higher than minority professors, all else held constant.** </li> 
...
There might be atype above.  enthinicty has a coefficient of 0.1234929
...
m2_ethnicity = lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m2_ethnicity)$adj.r.squared#0.1602303
```
