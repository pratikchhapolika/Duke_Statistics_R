...
```{r pick-model}
# type your code for the Exercise here, and Knit
#m3 Multiple R-squared:  0.6561 > Multiple R-squared:  0.3729
#m3 is superior
```
...
batAvg <- m3
summary(atBats)#Multiple R-squared:  0.3729
summary(hits)#Multiple R-squared:  0.6419
summary(wins)#Multiple R-squared:  0.361
summary(batAvg)#Multiple R-squared:  0.6561
```
...
ggplot(data = onBasePercent, aes(x = .fitted, y = .resid)) + geom_point() + geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") + ylab("Residuals")
#Nearly Normal Residucals satisfied:
