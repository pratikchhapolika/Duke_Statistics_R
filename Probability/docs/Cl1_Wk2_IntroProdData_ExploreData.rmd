...
ggplot(data = sfo_feb_flights, aes(x = dep_delay)) + geom_histogram(binwidth=5)

sfo_feb_flights %>% summarise(mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), n = n())
#ans: c is false
```
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(mean_dd = median(arr_delay), IQR_dd = IQR(arr_delay), n = n())
#ans:  Delta and United
```
...
Answer:  C?  See plot below - always skewed
...
#Same code as above.  LGA on time percentage is 72.8%
```
...
#ans:  There is an overall postive association between distance and average speed
```
...
nycflights <- nycflights %>% mutate(arr_type = ifelse(arr_delay < 1, "on time", "delayed"))
#What proportion of flights that were "delayed" departing arrive "on time"" gives the formula
#(delayed departing and arrived on time)/ (all flights delayed departing)

sum1 <- nycflights %>% summarise(sum(dep_type =="delayed" & arr_type=="on time"))
sum2 <- nycflights %>% summarise(sum(dep_type == "delayed"))
proportion <- (sum1/sum2)
proportion
#ans:   0.1833639
```