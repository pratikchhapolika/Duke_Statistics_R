...
The data used in this analysis is from the General Social Survey (GSS).  It is a sociological survey used to collect data on demographic characteristics and attitudes of residents of the United States. The survey is a representative sample of "English speaking persons 18 years of age or over, living in non-institutional arrangements within the United States". The GSS has been conducted since 1972.

This analysis is a retrospective observational study seeking to determine if there is a relationship between the frequency people participate in religious services - a measure of how religious they are - and their view on the government funding space exploration.  job. It is not an experiment. No causality inferences to the general population are possible - it is not an experiment. 

The two variables that I will be looking at are both categorical variables. *attend* which provides data on the GSS Survey the respondents participation in religious services.  The other variable, *natpsac*, which indicates the respondents thoughts on the amount of government spending which goes to the space program.
...
1. **Independence**: Sampled observations must be independent:
   + Random sample/assignment - This data is random based on the description of the data collection methods in the GSS FAQ.  While there are potential source of bias in GSS, it is generally accepted that GSS saisfies this random sample requirement.  *"..the GSS tries to follow the highest survey standards in design, sampling, interviewing, processing, and documentation. Items are designed by leading specialists in their field and then pretested, full-probability sampling is used, a high response rate is obtained, and many data quality checks from validation to verification are employed."*.  [Link](http://www.ssric.org/trd/other/ssda/gssdesc) 
   ...
   + If sampling without replacement, n<10% of popultion - The number of cases is less than ten percent of the population of the US ([323,987,000](http://www.worldometers.info/world-population/us-population/)).
   + Each case only contributes to one cell in the table (the data is not paired).  There is no evidence that suggestes a violation of this rule.
2. **Sample size**: Each cell must have at least 5 expected cases - A table below demosntrates that each cell has many more than 5 cases.
...
summary(modData)
```
Arbitrarily on my part, I am going to limit the analysis data to date no older than the year 2000.  Aft the code runs, the summary is re-displayed to ensure the operation was successful.
```{r filterYear}
...
The overall frequent attendance rate is `r tmpTable[2,4]/tmpTable[3,4]`.

If in fact religious service attendance and support for space are independent (i.e. if in fact $H_{0}$ is true), how many of the persons supporting space exploration would we expect to frequently attend religious services? (Note:  The expected values are shown in <span style="color:blue">blue</span>.)
...
We could calculate this by hand using the equation provided previously but that would be laboroious compared to the simplicity provided by R and the chisq.test used below.
...
By rejecting the null hypothesis $H_{0}$, test results provides strong evidence suggesting that the frequency of religious services and support for space exploration are dependent or have some association.  Recall that this is an alaysis of observational data so no causal connections can be made.
