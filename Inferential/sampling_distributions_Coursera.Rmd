...
#The distribution appears normal, not right-skewed like the population histogram.
```
...
#Not ethe use of rep_sample_n - dod not know that one!
...
dim(sample_means50)    
#The sampling distribution appears near normal, as expected.
sample_means50 %>% summarise(x_bar = mean(x_bar))
```
...
sample_means_small <- ames %>%
                    ...
                    summarise(x_bar = mean(area))
ggplot(data = sample_means_small, aes(x = x_bar)) + geom_histogram(binwidth = 20)
...
ames %>% sample_n(size = 50) %>% summarise(price = mean(price))#Answer:  183192.2
```
...
#Plot appears right skewed.  Mean price = 180422
```
...
#Plot appears less skewed.  Mean price = 180737
```
...
# type your code for this Exercise here, and Run Document
ames %>% sample_n(size = 15) %>% summarise(price = mean(price))#Answer:  178363.3
```
...
#Distribution looks nearly norm and a little right skewed
#Mean is 181103 
```
...
summary(sample_means150)
#Distribution looks more normal than the previous
#Mean is 180852
```
