...
```{r set-seed}
set.seed(5202001)                 # make sure to change the seed
```
...
<ol>
<li> 95% of the time the true average area of houses in Ames, Iowa, will be in this interval. - WRONG </li>
...
<li> 95% of the houses in Ames have an area in this interval - WRONG. </li>
...
# type your code for the Exercise here, and Knit
CL; params
#1499.69 is in the calculated confidence levels:  1434.103 < 1499.69 < 1637.963
```
...
ci_99 <- ci_99 %>% mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))

ci_data_99 <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(ci_99$lower, ci_99$upper),
                      capture_mu = c(ci_99$capture, ci_99$capture))
