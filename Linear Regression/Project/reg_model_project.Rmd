...
I have a teenage daughter who sees movies with her friends every week.  Occasionally I take the gaggle of girls to the movires and see it with them - sitting in a different part of the theater of course!  Some of the movies are just terrtible - imn my opinion.  I am often amazed how frequently my perceptions differr form youg teenagers.  (I know I should not be surprised.)

Research Question:  What does it take to make a movie popular?  

I really curious.  Do the variables that contribute to movie popularity more closely match my views or the perceptions of a teenage?
...
Before any data analysis, understnading the data is critical.  Lets first look at the 32 variables available to us:

The data frame has 651 redords and 32 attributes:
...
str(movies)
```
The `audience_score` is the response variable for my analysis.  It is the variable we will solve for in our analysis.  Not all of the reamianing variables are interesting to answer my question.  I will also intentionally remove a few variables.  For example, I perfer to get a solution without regard to when the movie was produced.  Certainly older movies may be percieved differently by younger people compared to a mature audience but for the purposes of this project, I am removing the date-related variables.  
```{r selectVariables}
movies_filtered <- select(movies, c(-(thtr_rel_year:dvd_rel_day), -top200_box, -(actor1:rt_url)))
```
Because my question surronds movie popularity with an interest in undetsnading if the result more closely mirrow my perception or my daughter's perception, I will also filter the records removing Documentaries - my daughter is not going to watch one of those!  Also, I am specifically interested in movies in thheaters - not TV movies.  Lastly, I want only complete records.
```{r}
...
```
The histogram does appear a bit left skewed but I submit this is understnabale.  I submit that popular movies typcally get rated highly by a broad audience where a less popualr movies will always have a measureable following.
