...
```
There are still some variables that I do not need for my analysis.  For example, I have make the assertion that the name of the movie is not relevant.  Similarly, I do not need the `title_type` since I have already selected *Feature Film*.  The varible `critics_rating` is simply a categorical representation of `critics_score`, it too is removed from the data set.  Simliarly, `audience_rating` is a categorical representation of `audience_score` si it too is removed.  Lastly, I have asked my daughter and her friends if the studio that makes a movie makes any difference to them.  The answer was a resounding *No*.  I received the same response when I asked about the director of a movie.
```{r}
...
```
Take note of the distribution of the last four variables.  The number of `Yes` responses to `No` responses are very unbalanced.  Let's keep this in mind as we proceed.  We may decide to drop these variables in out model.
...
The histogram does appear a bit left skewed but this is understandable.  I submit that popular movies typcally get rated highly by a broad audience where a less popular movies will always have a measureable following.  The plot is nearly norm enough for us to consider. 
...
Because the typical pairs plot does not display very useful information when many categorical factrs are in the data set, I opted to use a modified pairs plot avaialble in the psyche package.  (Functions are primarily for multivariate analysis and scale construction using factor analysis, principal component analysis, cluster analysis and reliability analysis, although others provide basic descriptive statistics.)
```{r}
library(psych)
pairs.panels(movies_filtered)
```

The oval-shaped object in the scatter plots above are called correlation elipses.  They provide a visualization of how strongly the correlated values are.  The dot at the center of the elipse indicates the point of the mean value for the x axis and y axis variables.  The correlation between the two variables is indicated by the shape of the elipse.  An alsmost perfectly round oval indicates a very week correlation.

The curve drawn on the scatter plot is called a loess smooth.  It indicates the general relationship between the x axis and y axis variables.  Note the scatter plot bwtween runtime and audience score.  There appears ther may be a slight nonlinear trend indicateing there may be a higher audience score with short and longer movies with some in the center falling out of favor.  This finding could not have been found from correlations alone.
