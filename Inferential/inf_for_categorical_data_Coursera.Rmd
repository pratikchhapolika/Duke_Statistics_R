...
#Why does this give a different result? Must be rounding.  Do not like this method
...
- **Independence**:  Sampled observations must be indepedent (random sample/assignment) and if sampling without replacement, n < 10% of population
...
- Margin of error describes the upper and lower boundaries of a confidence interval about the mean.  It is 1/2 the total range of the confidence interval.
</div>
...
- The sample proportion can be assumed to be nearly normal if it is based on a random sample of independent observations and if both np ≥ 10 and n(1−p) ≥ 10. 
