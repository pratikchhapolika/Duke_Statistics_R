...
#qplot(habit, weight, data=nc_plot, geom=c("boxplot")) + geom_point(alpha=0.4)
ggplot(nc_plot, aes(x=habit, y=weight)) + geom_boxplot()
```

```{r pirateplot, message=FALSE, warning=FALSE}
#Just for fun
library(yarrr)
...
<li> $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} ne \mu_{non-smoking}$ </li>
<li> $H_0: \bar{x}_{smoking} = \bar{x}_{non-smoking}$; $H_A: \bar{x}_{smoking} > \bar{x}_{non-smoking}$ </li>
<li> $H_0: \bar{x}_{smoking} = \bar{x}_{non-smoking}$; $H_A: \bar{x}_{smoking} > \bar{x}_{non-smoking}$ </li>
<li> $H_0: \mu_{smoking} \ne \mu_{non-smoking}$;  $H_A: \mu_{smoking} = \mu_{non-smoking}$ WRONG</li>
</ol>
...
<div id="exercise">
**Exercise**: Calculate a new confidence interval for the same parameter at the 99% confidence level. Comment on the width of this interval versus the one obtained in the the previous exercise.
</div>
```{r weeks-ci-90}
...
mothers. Use a method of your choice, and explain how your method works.  Ans:  max(young = 34, min(older)=35)
```{r cutoffmature}
...
Question:  Does the average weight of babies born to white mothers the same as the average weight of babies born to non-white mothers with 95% confidence?

$H_0: \mu_{white} = \mu_{non-white}$; $H_A: \mu_{white} \ne \mu_{non-white}$

```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(y = weight, x=whitemom, data = nc, statistic = "mean", type = "ci", method = "theoretical", conf_level = .95)
```
Conclusion:  We are 95% confident that babies born to white mothers are on average 0.31 to 0.75 pounds heavier at birth than babies born to nonwhite mothers. 
